...
const PatientEvolution = ({ patientId, patientName }) => {
    ...
    // Carregar evoluções e métricas
    useEffect(() => {
        const fetchData = async () => {
            setLoading(true);
            try {
                const [evolutionsRes, metricsRes] = await Promise.all([
                    axios.get(`/api/evolutions/patient/${patientId}`),
                    axios.get('/api/evolutions/metrics')
                ]);
    ...
    useEffect(() => {
        ...
        const data = filteredEvolutions.map(evolution => {
            ...
            if (selectedMetric !== 'all') {
                const metricValue = evolution.metrics.get(selectedMetric) || 0;
            ...
            metrics.forEach(metric => {
                if (selectedType === 'all' || metric.type === selectedType) {
                    metricValues[metric.name] = evolution.metrics.get(metric._id) || 0;
                }
            });
